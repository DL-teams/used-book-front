        ...
        <div class="w-box">
          ...
          <div class="right-box">
            ...
            <div class="nav-aside" ref="aside" :class="{fixed:st}">
                ...
                <div class="nav-user-wrapper pa" v-if="login">
                  <div class="nav-user-list">
                    <ul>
                      ...
                      <li>
                        <div @click="recharge()">账户充值</div>
                      </li>
                      <li>
                        <div @click="openSignIn()">签到</div>
                      </li>
                      <li>
              ...
              <div style="margin-left: 20px;color: #c8c8c8" v-if="this.$store.state.userInfo !== null">
                <div>余额:{{balance}}</div>
                <div>积分:{{points}}</div>
              </div>
            </div>
          ...
          <div class="nav-sub-wrapper" :class="{fixed:st}">
            <div class="w">
              <ul class="nav-list2">
                ...
                <li>
                  <a @click="changGoods(-2)" :class="{active:choosePage===-2}">二手商品</a>
                </li>
                <li>
                  <a @click="changGoods(-3)" :class="{active:choosePage===-2}">官方商品</a>
                </li>
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      changePage (v) {
        this.choosePage = v
        setStore('searchPage', v)
        this.$store.commit('changeSearchPage', v)
      },
      changGoods (v, item) {
        this.changePage(v)
        if (v === -1) {
          this.$store.commit('changeSearchPage', -1)
          this.$router.push({
            path: '/'
          })
        } else if (v === -2) {
          setStore('searchPage', 2)
          this.$store.commit('changeSearchPage', 2)
          this.$router.push({
            path: '/refreshgoods'
          })
        } else if (v === -3) {
          setStore('searchPage', 4)
          this.$store.commit('changeSearchPage', 4)
          this.$router.push({
            path: '/refreshofficialgoods'
          })
        } else {
          // 站内跳转
          if (item.type === 1) {
      ...
      // 控制顶部
      navFixed () {
        if (this.$route.path === '/goods' || this.$route.path === '/home' || this.$route.path === '/goodsDetails' || this.$route.path === '/thanks' || this.$route.path === '/officialgoods' || this.$route.path === 'officialgoodsDetails') {
    ...
    mounted () {
      if (this.$store.state.userInfo !== null) {
        this.balance = this.$store.state.userInfo.info.balance
        this.points = this.$store.state.userInfo.info.points
      }
